...
var import_http_status_codes = __toESM(require("http-status-codes"));
...
var CanisterClient = class {
  ...
  createAgent(hostUrl, identity) {
    const agent = new import_agent.HttpAgent({
      host: hostUrl,
      identity
    });
    return agent;
  ...
  createActor(canisterId) {
    return import_agent.Actor.createActor(idlFactory, {
      agent: this.agent,
      canisterId
    });
  }
  ...
  async query(method, pathname, config) {
    ...
    if (result.status_code >= 400) {
      const error = {
        ...
        statusText: import_http_status_codes.default.getStatusText(result.status_code),
